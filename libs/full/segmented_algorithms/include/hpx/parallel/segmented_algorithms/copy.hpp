...
namespace hpx::segmented {
    ...
    ///////////////////////////////////////////////////////////////////////////
    // segmented copy_if — tag_invoke overloads
    //
    // copy_if must write accepted elements contiguously into the destination.
    // Because the number of elements accepted per source segment is not known
    // until the segment has been scanned, we cannot reset the destination
    // iterator for each segment independently (that would cause gaps or
    // overwrites). We therefore use a simple sequential element-by-element
    // fallback that correctly advances a single destination cursor.

    // no-policy (sequential) overload
    template <typename SegIter, typename OutIter, typename Pred>
        requires(hpx::traits::is_iterator_v<SegIter> &&
            hpx::traits::is_segmented_iterator_v<SegIter> &&
            hpx::traits::is_iterator_v<OutIter> &&
            hpx::traits::is_segmented_iterator_v<OutIter>)
    OutIter tag_invoke(
        hpx::copy_if_t, SegIter first, SegIter last, OutIter dest, Pred&& pred)
    {
        static_assert(hpx::traits::is_input_iterator_v<SegIter>,
            "Requires at least input iterator.");
    ...
    // execution-policy overload — sequential policies run the same loop;
    // parallel policies also run sequentially here because a truly parallel
    // segmented copy_if requires a two-phase (count + scan) approach that is
    ...
    template <typename ExPolicy, typename SegIter, typename OutIter,
        ...
        requires(hpx::is_execution_policy_v<ExPolicy> &&
            ...
            hpx::traits::is_segmented_iterator_v<OutIter>)
    hpx::parallel::util::detail::algorithm_result_t<ExPolicy, OutIter>
    tag_invoke(hpx::copy_if_t, ExPolicy&& /* policy */, SegIter first,
        SegIter last, OutIter dest, Pred&& pred)
    {
        static_assert(hpx::traits::is_forward_iterator_v<SegIter>,
            "Requires at least forward iterator.");

        using result =
            hpx::parallel::util::detail::algorithm_result<ExPolicy, OutIter>;
