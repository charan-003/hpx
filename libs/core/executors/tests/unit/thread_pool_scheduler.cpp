...
void test_bulk_starts_on()
{
    ...
    for (int n : ns)
    {
        ...
        // Use starts_on to provide scheduler through environment
...
struct test_scheduler_env
{
    ex::thread_pool_scheduler sched;

    friend auto tag_invoke(
        ex::get_scheduler_t, test_scheduler_env const& self) noexcept
    {
        return self.sched;
    }
};
...
void test_stdexec_domain_queries()
{
    ...
    {
        test_scheduler_env env{scheduler};
...
void test_completion_scheduler()
{
...

    {
        auto sender = ex::starts_on(
            ex::thread_pool_scheduler{}, ex::just(42));
        auto completion_scheduler =
            ex::get_completion_scheduler<ex::set_value_t>(ex::get_env(sender));
        static_assert(
            std::is_same_v<std::decay_t<decltype(completion_scheduler)>,
